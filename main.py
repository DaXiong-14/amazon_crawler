...
def selection_core(cid, site):
    ...
    params = {
        'marketId': _get_marketId(site),  # 4 德国站
    ...
    try:
        filename = os.path.join(os.getcwd(), f'temp\\selection\\{cid}_{site}_{current_time}.xlsx')
...
def category_integration_start(site="US", method='f'):
    ...
    if method == 'f':
        ...
        for k in result_dict.keys():
            ...
            url = f'http://{host}:{PORT}/api/crawler/cn'
...
if __name__ == '__main__':
    # selection_start()
    category_integration_start(method='m')
